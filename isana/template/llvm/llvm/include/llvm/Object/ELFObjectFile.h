...
Expected<uint32_t> ELFObjectFile<ELFT>::getSymbolFlags(DataRefImpl Sym) const {
  ...
  } else if (EF.getHeader().e_machine == ELF::EM_{{ XPU }}) {
    if (Expected<StringRef> NameOrErr = getSymbolName(Sym)) {
      StringRef Name = *NameOrErr;
      if (Name.starts_with("$d") || Name.starts_with("$x"))
        Result |= SymbolRef::SF_FormatSpecific;
...
StringRef ELFObjectFile<ELFT>::getFileFormatName() const {
  ...
  case ELF::ELFCLASS32:
    ...
    case ELF::EM_XTENSA:
      return "elf32-xtensa";
    case ELF::EM_{{ XPU }}:
      return (IsLittleEndian ? "elf32-{{ xpu }}-little" : "elf32-{{ xpu }}-big");
    default:
  ...
  case ELF::ELFCLASS64:
    ...
    case ELF::EM_{{ XPU }}:
      return (IsLittleEndian ? "elf64-{{ xpu }}-little" : "elf64-{{ xpu }}-big");
    default:
...
template <class ELFT> Triple::ArchType ELFObjectFile<ELFT>::getArch() const {
  ...
  case ELF::EM_{{ XPU }}:
    switch (EF.getHeader().e_ident[ELF::EI_CLASS]) {
    case ELF::ELFCLASS32:
      return IsLittleEndian ? Triple::{{ xpu }}32le : Triple::{{ xpu }}32be;
    case ELF::ELFCLASS64:
      return IsLittleEndian ? Triple::{{ xpu }}32le : Triple::{{ xpu }}64be;
    default:
      report_fatal_error("Invalid ELFCLASS!");
    }
